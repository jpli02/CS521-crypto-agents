...
def get_paths(args):
    price_path = f'data/btc/price.csv'
    txn_path = f'data/btc/txn_stat.csv'
    news_dir = f'data/btc/news'
    timecol = 'date'
    price_timefmt = '%Y-%m-%d'
    txn_timefmt = '%Y-%m-%d'
...
class BTCTradingEnv:
    ...
    def get_close_state(self, today, next_day, first_day=False):
        ...
        close_net_worth = self.cash + self.btc_held * next_open_price
    ...
    def reset(self):
        ...
        self.btc_held = (self.starting_net_worth - self.cash) / self.starting_price
    ...
    def step(self, action):
        ...
        if -1 <= action < 0 and self.btc_held > 0:
            btc_diff = abs(action) * self.btc_held
            cash_diff = btc_diff * open_price
            self.btc_held -= btc_diff
            self.cash += cash_diff
            self.cash -= GAS_FEE * open_price + cash_diff * EX_RATE
        if 0 < action <= 1 and self.cash > 0:
            cash_diff = abs(action) * self.cash
            btc_diff = cash_diff / open_price
            self.cash -= cash_diff
            self.btc_held += btc_diff
            self.cash -= GAS_FEE * open_price + cash_diff * EX_RATE

...
from sol_env import SOLTradingEnv
...
def get_parser():
    ...
    parser.add_argument("--num_envs", type=int, default=4, help="The number of environments per group")
    parser.add_argument("--run_name", type=str, default='crypto_run', help="The name of the run")
...
def main(args) -> None:
    ...
    if args.is_resume:
        ...
        # initialize environment configs for each group
        env_configs: List[Dict[str, Any]] = []
        for crypto in ['btc', 'eth', 'sol']:
            for i in range(args.num_envs):
                env_configs += [{
                    'name': f'{crypto}_env_{i}',
                    'crypto': crypto,
                    'memory': [],
                    'is_success': False,
                    'skip': False
    ...
    while trial_idx < args.num_trials:
        ...
        # Group environments by cryptocurrency
        btc_envs = [env for env in env_configs if env['crypto'] == 'btc']
        eth_envs = [env for env in env_configs if env['crypto'] == 'eth']
        sol_envs = [env for env in env_configs if env['crypto'] == 'sol']

        # Run trials for each group
        if btc_envs:
            btc_env = BTCTradingEnv(args)
            run_trial(trial_log_path, world_log_path, trial_idx, btc_envs, args=args)
            btc_env.close()

        if eth_envs:
            eth_env = ETHTradingEnv(args)
            run_trial(trial_log_path, world_log_path, trial_idx, eth_envs, args=args)
            eth_env.close()

        if sol_envs:
            sol_env = SOLTradingEnv(args)
            run_trial(trial_log_path, world_log_path, trial_idx, sol_envs, args=args)
            sol_env.close()
